/**
 * title - terminal title integration.
 *
 * Uses the same logic as the remote extension:
 * - dynamic title with project, session, and model info
 * - braille spinner while the agent is working
 * - Ghostty progress bar integration when supported
 */
...
function writeToTerminal(seq: string) {
	try {
...
function setProgress(state: number, value?: number) {
	...
	writeToTerminal(`\x1b]9;4;${args}\x07`);
}
