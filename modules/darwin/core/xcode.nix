  ...
  system.activationScripts.extraActivation.text = ''
    (
      ...
      if command -v xcodes >/dev/null 2>&1; then
        ...
        run_as_user() { sudo -u ${username} -H env PATH="$PATH" "$@"; }
        latest_xcode=$(run_as_user xcodes update 2>/dev/null | grep -E '^[0-9.]+ \(' | tail -1 | awk '{ print $1 }')
        if [ -n "$latest_xcode" ]; then
          if ! run_as_user xcodes installed 2>/dev/null | grep -q "^$latest_xcode ("; then
            echo "installing Xcode $latest_xcode..." >&2
            run_as_user xcodes install "$latest_xcode" --no-superuser --experimental-unxip --empty-trash \
              || echo "warning: could not install Xcode $latest_xcode; run: xcodes install $latest_xcode --select" >&2
          fi
          xcode_path=$(run_as_user xcodes installed 2>/dev/null | awk -F'\t' -v v="$latest_xcode" 'index($1, v " (") == 1 { print $NF }')
